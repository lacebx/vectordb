...
def test_hnswlib_vectordb_single_query(docs_to_index, limit, replicas, shards, protocol, tmpdir):
    ...
    port = random_port()
    with HNSWVectorDB[MyDoc](ef=5000).serve(workspace=str(tmpdir), replicas=replicas, shards=shards, port=port, protocol=protocol,
                                             timeout_ready=10000) as db:
        db.index(inputs=docs_to_index)
        if replicas > 1:
            time.sleep(2)
...
def test_hnswlib_vectordb_restore(docs_to_index, replicas, shards, protocol, tmpdir):
    ...
    with HNSWVectorDB[MyDoc]().serve(workspace=str(tmpdir), replicas=replicas, shards=shards, port=port, protocol=protocol,
                                      uses_with={'ef': 5000}, timeout_ready=10000) as new_db:
        time.sleep(2)
